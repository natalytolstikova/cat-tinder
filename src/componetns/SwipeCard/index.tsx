...
export interface ISwipeCard<T> {
    children: (item: T, swipe: Animated.ValueXY, isFirst: boolean) => React.ReactNode;
    items: T[];
    setItems: (fun: TPrevStateAct) => void;
    renderActionBar: (handleChoice: (direction: number) => void) => React.ReactNode;
    onSwipeUser: (swipe: Animated.ValueXY, prevState: T[]) => void;
}

export const SwipeCard = <T,>({ children, items, setItems, renderActionBar, onSwipeUser }: ISwipeCard<T>) => {
